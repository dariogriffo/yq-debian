...
if [ -z "$yq_VERSION" ] || [ -z "$BUILD_VERSION" ]; then
    echo "Usage: $0 <yq_version> <build_version>"
    echo "Example: $0 0.11.21 1"
    exit 1
fi

PACKAGE_NAME="yq"
ORIG_TARBALL="${PACKAGE_NAME}_${yq_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${yq_VERSION}"

echo "Creating Debian/Ubuntu source packages for yq ${yq_VERSION}-${BUILD_VERSION}..."
...
if [ ! -f "$ORIG_TARBALL" ]; then
    ...
    wget -q "https://github.com/mikefarah/yq/archive/refs/tags/v${yq_VERSION}.tar.gz" -O "$ORIG_TARBALL"
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${yq_VERSION}-${BUILD_VERSION}~${dist}"
    ...
    rm -rf "$BUILD_DIR"
    tar -xf "$ORIG_TARBALL"
    # GitHub archives extract as yq-4.x.y/ which matches our BUILD_DIR
...
yq (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${yq_VERSION}.
