...
build_architecture() {
    ...
    for dist in "${arr[@]}"; do
        FULL_VERSION="$yq_VERSION-${BUILD_VERSION}~${dist}_${build_arch}_ubu"
        echo "  Building $FULL_VERSION"

        if ! docker build . -f Dockerfile.ubu -t "yq-ubuntu-$dist-$build_arch" \
        ...
        id="$(docker create "yq-ubuntu-$dist-$build_arch")"
